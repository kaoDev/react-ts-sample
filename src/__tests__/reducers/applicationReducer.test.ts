...
describe('application state reducer handles actions correctly', () => {
  ...
  const appStateWithNewText: ApplicationState = { text: changedText }

  test('do nothing on INPUT_CHANGED', () => {
    expect(reduceApplicationState(appState, inputChanged(changedText))).toBe(
      appState
    )
  })

  test('change text value on TEXT_CHANGED', () => {
    expect(reduceApplicationState(appState, textChanged(changedText))).toEqual(
      appStateWithNewText
    )
  })
})
